...
@bot.message_handler(func=lambda message: message.chat.id in user_step, content_types= ["text", "photo"])
def handle_message(message):
    try:
        ...
        if step == 'name':
            ...
            with project_login.app_context():
                pro_duct = Product(name=product['name'], price=product['price'], discount=product['discount'], description=product['description'], image_url=f"/static/images/{product['name']}.png")
